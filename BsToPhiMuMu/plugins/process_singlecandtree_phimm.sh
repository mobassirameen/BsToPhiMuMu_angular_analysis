...
do
    ...
    ./sel mc.lite mc cutopt root://cmseos.fnal.gov//eos/uscms/store/user/ckar/BsToMuMuPhi-SignalMC_2016MINI_v1_Correction_Iso.root BsToPhiMuMu_OfficialMC_signal_2016Mini_Presel_dimuon_v1 -s $((2000000*$((num-1))))  -n 10000
    #echo -e "\n==> file$num is done."
...
do
    ./sel mc.lite mc cutopt root://cmseos.fnal.gov//eos/uscms/store/user/ckar/BsToMuMuPhi-SignalMC_2016MINI_v2_Correction_Iso.root BsToPhiMuMu_OfficialMC_signal_2016Mini_Presel_dimuon_v2 -s $((2000000*$((num-1))))  -n 10000
    echo -e "\n==> file$num is done."
    echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
done
